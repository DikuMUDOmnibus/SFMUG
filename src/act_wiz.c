...
void echo_to_room( sh_int AT_COLOR, ROOM_INDEX_DATA *room, char *argument )
{
    ...
    /* can't echo to vnum -1 ..ships -Lews */
    if ( room == NULL )
    	return;
...
void do_mstat( CHAR_DATA *ch, char *argument )
{
    ...
    /* randomly created mobs cause core dumps -Lews */
    if ( IS_NPC(victim) && ( victim->pIndexData->vnum > MOB_VNUM_FIRST_RANDOM_MOB && victim->pIndexData->vnum < MOB_VNUM_LAST_RANDOM_MOB ) )
    {
    	send_to_pager( "Randomly generated mobs can not be looked at.\n\r", ch );
    	return;
    }
...
void do_newbieset( CHAR_DATA *ch, char *argument )
{
	...
	return;
     ...
     || (victim->class == CLASS_SOLDIER) || (victim->class == CLASS_PILOT)
     || (victim->class == CLASS_HACKER) )
     {
...
/* Added the burlap sack to the newbieset.  The sack is part of sgate.are
   called Spectral Gate.  Brittany */

     {
       ...
       OBJ_INDEX_DATA *obj_ind = get_obj_index( OBJ_VNUM_BAG );
       if ( obj_ind != NULL )
       {
         obj = create_object( obj_ind, 1 );
         obj_to_char( obj, victim );
