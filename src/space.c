...
void update_space( )
{
   ...
   {    
        ...
        if (ship->shipstate == SHIP_DESTROYED)
          destroy_ship(ship , NULL);
        ...
        if (ship->shipstate == SHIP_HYPERSPACE)
        {
            ...
            {
		   if (ship->energy < (100+ship->hyperdistance) )
		   {
			echo_to_cockpit( AT_RED, ship, "Insufficiant Energy. Ship lost in Hyperspace.");
			ship->shipstate = SHIP_LOST;
		   }
		   else
		   {
	  ...
	  if (ship->shipstate == SHIP_LOST)
	  {
          if ( ship->energy > 0 && ship->shipstate == SHIP_DISABLED && ship->class != SPACE_STATION )
             ship->energy -= 100;
          else if ( ship->energy > 0 )
             ship->energy += ( 10 + ship->model*2 );
          else 
             destroy_ship(ship , NULL);
	  }
   ...
   for ( ship = first_ship; ship; ship = ship->next )
   {
       ...
       if ( autofly(ship) )
       {
          ...
          else if ( ( ship->class == CAPITAL_SHIP || ship->class == SPACE_STATION 
       ...
       if ( ( ship->class == CAPITAL_SHIP || ship->class == SPACE_STATION )
       && ship->target == NULL )
       {
...
void write_ship_list( )
{
	...
	/* Lets sort ships by class, so that we don't have ships trying to dock in ships that	*
	 * don't exist yet -Lews 3/02											*/

/* Space Stations */
    for ( tship = first_ship; tship; tship = tship->next )
    if ( tship->class == SPACE_STATION )
      fprintf( fpout, "%s\n", tship->filename );
/* Capital Ships */
    for ( tship = first_ship; tship; tship = tship->next )
    if ( ( tship->type != MOB_SHIP && tship->class == CAPITAL_SHIP ) )
      fprintf( fpout, "%s\n", tship->filename );
/* the rest of the ships */
    for ( tship = first_ship; tship; tship = tship->next )
    if ( tship->type != MOB_SHIP && tship->owner && tship->owner[0] != '\0' 
		&& tship->class != SPACE_STATION && tship->class != CAPITAL_SHIP )
      fprintf( fpout, "%s\n", tship->filename );
...
SHIP_DATA * ship_in_room( ROOM_INDEX_DATA *room, char *name )
{
    ...
    /* make capital class ships always visible -lews 11/01 */
    ...
    for ( ship = room->first_ship ; ship ; ship = ship->next_in_room )
     if ( !str_cmp( name, ship->name ) )
      if ( ship->owner && ship->owner[0] != '\0' )
       ...
       || ship->class == CAPITAL_SHIP || ship->class == SPACE_STATION )  
         return ship;
...
/* 
 * Gets the pointer to the ships hanger structure from ship name
 */
ROOM_INDEX_DATA *get_ship_hanger( char *name )
{
  SHIP_DATA *ship;
  ROOM_INDEX_DATA *hanger;
...
SHIP_DATA *ship_from_hanger( ROOM_INDEX_DATA * room )
{
   SHIP_DATA *ship;
...
void fread_ship( SHIP_DATA *ship, FILE *fp )
{
	...
	case 'E':
		...
		if ( ( ship->lastdoc <= 0 ) && ( !ship->on_ship )) 
	...
	case 'N':
        ...
        case 'O':
            ...
            KEY( "On_ship",          ship->on_ship,            fread_string( fp ) );
            break;
...
bool load_ship_file( char *shipfile )
{
    ...
    {      
       ...
       if ( ship->class == SPACE_STATION || ship->type == MOB_SHIP 
		|| ( ship->class == CAPITAL_SHIP && ( ship->home != STRALLOC( "" ) ) ) )
       {
	    ...
	    { 
		if( ship->on_ship != STRALLOC( "" ))
		{
             docked = get_ship(ship->on_ship);
             ship_to_hanger( ship, docked->hanger ); /* put the ship in the hanger.. */
             ship->location = 0;
             ship->lastdoc =0;
         	}
         	else
		  ...
		  if( ship->lastdoc != 0 )
         	  {
               ship_to_room( ship , ship->lastdoc ); /* ..or put the ship in the last docked room  -Lews */
               ship->location = ship->lastdoc;
               ship->docked = NULL;
		  }
		  else
		  {
               ship_to_room( ship , ship->shipyard ); /* ..or put the ship in the shipyard  -Lews */
...
void resetship( SHIP_DATA *ship )
{
     ...
     if ( ship->class != SPACE_STATION && ship->type != MOB_SHIP  && 
          ( ship->class != CAPITAL_SHIP && ship->home != NULL ) )
     {
           extract_ship( ship );
...
{    
     ...
     if ( ( ship->class == SPACE_STATION || ship->class == CAPITAL_SHIP || ship->type == MOB_SHIP )
		&& ship->home )
     {
          ship_to_starsystem(ship, starsystem_from_name(ship->home) );  
          ship->vx = number_range( -5000 , 5000 );
...
void do_setship( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( "lastdoc class onship\n\r", ch );
	return;
    }
    ...
    if ( !str_cmp( arg2, "onship" ) )
    {
	STRFREE( ship->on_ship );
	ship->on_ship = STRALLOC( argument );
	ship->lastdoc = 0;
	send_to_char( "Done.\n\r", ch );
	save_ship( ship );
	return;
...
{   
    ...
    ch_printf( ch, "&O%s %s : &Y%s\n\r&OFilename: &Y%s\n\r",
		       ...
		       (ship->class == LAND_VEHICLE ? "land vehicle" : "Unknown" ) ) ) ) ) ), 
    			ship->name,
    			ship->filename);
    ch_printf( ch, "&OHome: &Y%s\n\r&OOwner: %s   &OPilot: &Y%s   &OCopilot: &Y%s\n\r",
    			ship->home,
    			ship->owner, ship->pilot,  ship->copilot );
    ch_printf( ch, "&OLocation: &Y%d   &OLastdoc: &Y%d   &OShipyard: &Y%d\n\r",
    			ship->location,
    			ship->lastdoc,
    			ship->shipyard);
    ch_printf( ch, "&ODocked: &Y%s  &OHanger: &Y%s\n\r",
    			ship->on_ship,
			ship->hanger != NULL ? "yes" : "no" );
    ch_printf( ch, "&OTractor Beam: &Y%d  ",
    			ship->tractorbeam);
    ch_printf( ch, "&OLasers : &Y%d  &OLaser Condition: &Y%s\n\r",
    			ship->lasers,
    			ship->laserstate == LASER_DAMAGED ? "Damaged" : "Good");		
    ch_printf( ch, "&OMissiles: &Y%d/%d  &OCondition: &Y%s\n\r",
       			ship->missiles,
    			ship->maxmissiles,
    			ship->missilestate == MISSILE_DAMAGED ? "Damaged" : "Good");		
    ch_printf( ch, "&OHull: &Y%d/%d  &OShip Condition: &Y%s\n\r",
                        ship->hull,
    		        ship->maxhull,	
    			ship->shipstate == SHIP_DISABLED ? "Disabled" : "Running");
    		
    ch_printf( ch, "&OShields: &Y%d/%d   &OEnergy(fuel): &Y%d/%d   &OChaff: &Y%d/%d\n\r",
                        ship->shield,
    		        ship->maxshield,
    		        ship->energy,
    		        ship->maxenergy,
    ...
    ch_printf( ch, "&OSpeed: &Y%d/%d   &OHyperspeed: &Y%d\n\r  &OManueverability: &Y%d",
...
void do_ships( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_NPC(ch) )
    {
      ...
      {   
        ...
        else if ( ship->starsystem )
	  {
 		ch_printf( ch, " %s \t\t\t -%s System\n\r", ship->name, ship->starsystem->name );
	  }
	  else
          ch_printf( ch, " %s\n\r", ship->name );
...
bool check_pilot( CHAR_DATA *ch , SHIP_DATA *ship )
{
   ...
   {        
      ...
      {
        if ( nifty_is_name(ch->name, ch->pcdata->clan->leader) )
          return TRUE;
...
{   
    ...
    echo_to_ship( AT_WHITE, ship , "And then darkness....");
     ...
     for ( room = ship->first_room ; room ; room = room->next_in_ship )
     {
       ...
       /* take care of ships docked in the hanger -Lews 11/01 */
        if( ship->class == CAPITAL_SHIP || ship->class == SPACE_STATION )
         for ( rship = room->first_ship ; rship ; rship = rship->next_in_room )
         {
           rship->shipstate = SHIP_DESTROYED;
         }
...
bool ship_to_hanger(SHIP_DATA *ship , ROOM_INDEX_DATA *shipto )
{
...
void do_leaveship( CHAR_DATA *ch, char *argument )
{
   ...
   /* taken out for hangers -Lews
    if ( ship->lastdoc != ship->location && ship->shipstate != SHIP_DOCKED )
    {
    ...
    */
    ...
    /*Exit on to a hanger or just out of the ship */
    if ( ( toroom = get_room_index( ship->location ) ) != NULL ) 
    {
     ...
     else if ( ship->on_ship )
     {
            target = get_ship( ship->on_ship );
            toroom = target->hanger;
            act( AT_PLAIN, "$n exits the ship.", ch,
		NULL, argument , TO_ROOM );
	    act( AT_PLAIN, "You exit the ship.", ch,
		NULL, argument , TO_CHAR );
...
/*   His is taken out until I get hangers working -Lews 9/01
    SHIP_DATA *onship;
    HANGER_DATA *bayopen;

	if ( ( on_ship == ship_from_hanger( ship->lastdoc ) )  != NULL  && ! onship->bayopen )
	{
...
*/
             ...
             /* check if ship is in the same room as it landed -taken out for hangers -Lews
              ...
              */
                ...
                if ( number_percent( ) < chance )
    		{  
               /* ship launch price is the class * 100 + energy .2 + hull + missles & damage - Lews */
			price = ( (1+ship->class) * 100 );

                  price += ( ( ship->maxenergy-ship->energy ) * 0.2);
    	          ...
    	          if ( ch->pcdata && ch->pcdata->clan && !str_cmp(ch->pcdata->clan->name,ship->owner) ) 
                  {
                   ...
                   ch_printf(ch, "&GIt costs %s %ld gold to ready this ship for launch.\n\r", ch->pcdata->clan->name, price );   
                  }
                  else
                  {
                   if ( ch->gold < price )
                   ...
                   ch->gold -= price;
                   ch_printf(ch, "&GYou pay %ld gold to ready the ship for launch.\n\r", price );   
...
void do_land( CHAR_DATA *ch, char *argument )
{
    ...
    bool rfound = FALSE;
               ...
               if (ship->shipstate == SHIP_LOST)
               {
                  send_to_char("&RWhere? You are Lost!\n\r",ch);
                  return;   
               }
    	        ...
    	        {  
    	           ...
    	           for ( target = ship->starsystem->first_ship; target; target = target->next_in_starsystem )
                   {       
                        if ( ( target->class == SPACE_STATION || target->class == CAPITAL_SHIP )
                               && target != ship  ) 
                ...
                if ( !pfound )
                {
                   ...
                   if( target->hanger == NULL )
                   {
                    ...
                    for ( hanger = target->first_hanger ; hanger ; hanger = hanger->next )
                    {
    	              ...
    	              if ( target->class != SPACE_STATION && target->class != CAPITAL_SHIP )
    	              {
    	                send_to_char("&RThat ship does not have a hanger for you to land in!\n\r",ch);
    	                return;
    	              }
                   */
...
{    
    ...
    for ( target = ship->starsystem->first_ship; target; target = target->next_in_starsystem )
    {        
       if ( !str_prefix( arg1 , target->name) )
       {
         docked = target->hanger;
         ship->on_ship = STRALLOC( target->name );
         break;
       }
  ...
  /* testing */   
    sprintf( buf ,"Docking: %s\n" , ship->on_ship  );
    echo_to_ship( AT_GREEN , ship , buf );

   if( ship_to_hanger( ship, docked ) )
   {
     echo_to_ship( AT_YELLOW , ship , "The ship passes though the outer hull.");
     echo_to_room( AT_YELLOW , target->hanger, "The ship lands before you.");
     ship->on_ship = STRALLOC( target->name );
     ship->location = 0; /* set these the same for other checks in functions */
     ship->lastdoc = 0;
     
   }
   else if ( ( ship->on_ship == NULL ) && !ship_to_room( ship , destination ) )
   {
    ...
    {
      echo_to_ship( AT_YELLOW , ship , "You feel a slight thud as the ship sets down in the hanger."); 
      sprintf( buf, "%s docks in the %s.", ship->name, ship->on_ship );
      ship->shipstate = SHIP_DOCKED;
...
void do_hyperspace(CHAR_DATA *ch, char *argument )
{
    ...
    char buf[MAX_STRING_LENGTH];
        ...
        if ( autofly(ship)  )
    	        ...
    	        if (ship->shipstate != SHIP_READY && ship->shipstate != SHIP_LOST)
    	        {
    	            send_to_char("&RPlease wait until the ship has finished its current manouver.\n\r",ch);
    	            return;
    	        } 
        ...
        if ( ship->energy < ( (200+ship->hyperdistance) * 1+ship->model ) )
        {
              send_to_char("&RTheres not enough fuel!\n\r",ch);
              return;
        }
    ...
    ship->energy -= ( (100+ship->hyperdistance) * 1+ship->model );
...
/* I don't think we need this check yet -Lews
...
*/
	...
	chance = chance * ( ship->tractorbeam / (1+target->currspeed ) );
...
/* Taken out until I get hangers working -Lews 9/01
void do_openbay( CHAR_DATA *ch, char *argument )
{
	SHIP_DATA *ship;
	int chance;
