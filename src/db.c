...
void boot_db( bool fCopyOver )
{
	...
	*/
	...
	ASSIGN_GSN( gsn_pickshiplock  , "pickshiplock" );
        ...
        ASSIGN_GSN( gsn_slugs,          "slug throwers" );
        ASSIGN_GSN( gsn_blasters,       "blasters" );
        ASSIGN_GSN( gsn_slances,        "shock lances" );      
...
/*
 * Creates a simple exit  to a ship with no fields filled. -Lews 1/03
 * Exits are inserted into the linked list based on vdir.
 */
EXIT_DATA *make_ship_exit( ROOM_INDEX_DATA *pRoomIndex, ROOM_INDEX_DATA *to_room, sh_int door )
{
	EXIT_DATA *pexit, *texit;
	bool broke;
	...
	if ( to_room )
	{
	    texit = get_ship_exit_to( to_room, rev_dir[door] );
	    if ( texit )	/* assign reverse exit pointers */
	    {
		texit->rexit = pexit;
		pexit->rexit = texit;
