...
SPEC_FUN *spec_lookup( const char *name )
{
    ...
    if ( !str_cmp( name, "spec_medic"      ) ) return spec_medic;
    return 0;
}
...
char *lookup_spec( SPEC_FUN *special )
{
    ...
    if ( special == spec_medic          )       return "spec_medic";
    return "";
}
...
bool spec_medic( CHAR_DATA *ch )
{
    ...
    {
    case 0:
    act( AT_MAGIC, "$n says, Take two of these and call me in the morning.", ch, NULL, NULL, TO_ROOM );
	spell_smaug( skill_lookup( "armor" ), ch->level, ch, victim );
	return TRUE;

    case 1:
    act( AT_MAGIC, "$n pushes some buttons on his pad.", ch, NULL, NULL, TO_ROOM );
	spell_smaug( skill_lookup( "bless" ), ch->level, ch, victim );
	return TRUE;

    case 2:
    act( AT_MAGIC, "$n puts some drops in your eyes.", ch, NULL, NULL, TO_ROOM );
	spell_cure_blindness( skill_lookup( "cure blindness" ),
	    ch->level, ch, victim );
	return TRUE;

    case 3:
    act( AT_MAGIC, "$n wraps your wounds.", ch, NULL, NULL, TO_ROOM );
	spell_smaug( skill_lookup( "cure light" ),
	    ch->level, ch, victim );
	return TRUE;

    case 4:
    act( AT_MAGIC, "$n injects you with something.", ch, NULL, NULL, TO_ROOM );
	spell_cure_poison( skill_lookup( "cure poison" ),
	    ch->level, ch, victim );
	return TRUE;

    case 5:
    act( AT_MAGIC, "$n prics you with a needle.", ch, NULL, NULL, TO_ROOM );
	spell_smaug( skill_lookup( "refresh" ), ch->level, ch, victim );
	return TRUE;

    case 6:
    act( AT_MAGIC, "$n uses one of the machines to scan you.", ch, NULL, NULL, TO_ROOM );
        spell_smaug( skill_lookup( "cure serious" ), ch->level, ch, victim );
        return TRUE;

    case 7:
    act( AT_MAGIC, "$n performs some tests.", ch, NULL, NULL, TO_ROOM );
    act( AT_MAGIC, "$n Uses some ointment on you.", ch, NULL, NULL, TO_ROOM );
        spell_remove_curse( skill_lookup( "remove curse" ), ch->level, ch, victim );
        return TRUE;
