...
void fread_planet( PLANET_DATA *planet, FILE *fp )
{
	...
	case 'S':
	    ...
	    break;
	case 'T':
	    KEY( "Teclevel",	planet->tec_level,		fread_number( fp ) );
	    break;
...
void do_setplanet( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( "\n\rField being one of:\n\r", ch );
	send_to_char( " name filename starsystem governed_by tec_level\n\r", ch );
	return;
    }
    ...
    if ( !strcmp( arg2, "tec_level" ) )
    {
        if( atoi(argument) < 0 || atoi(argument) > 4 )
        {
	    send_to_char( "Tec_level must be between 0 - 4.\n\r", ch );
	    return;
	}
	
	planet->tec_level = atoi(argument);
	send_to_char( "Done.\n\r", ch );
	save_planet( planet );
	return;
    }
...
{   
    ...
    ch_printf( ch, "&WTechnology Level: &G%d\n\r", planet->tec_level );
