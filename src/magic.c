...
void do_cast( CHAR_DATA *ch, char *argument )
{
	...
	else
	{
     ...
     /* area stuff does not work on ships -Lews 
 ...
 *  added support for ships -lews 1/03
 *
...
ch_ret spell_portal( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	send_to_char("Where are you going to channel?\n\r", ch);
    	return rNONE;
    }
    
        /* so a caster can get to a ship */
       /* Board a ship -Lews 1/2003     */
	if ( ( ship = get_ship( target_name ) ) != NULL )
	{    
	
		if ( ship->class != SPACECRAFT || ship->class <= SPACE_STATION )
		{
   		if ( ( targetRoom = ship->hanger )  != NULL )
		{
			toship = 1;
			fromRoom = ch->in_room;
			...
			pexit = make_exit( fromRoom, targetRoom, DIR_PORTAL ); 
   			 ...
   			 {
			set_char_color( AT_MAGIC, ch );
			send_to_char("a glowing white line apears, and twists, until a portal apears!\n\r", ch);
   			 }
    			else
			act( AT_COLORIZE, skill->hit_char, ch, NULL, victim, TO_CHAR );
   			if ( !skill->hit_room || skill->hit_room[0] == '\0' )
			act( AT_COLORIZE, "$n channels as glowing white line apears, and twists, until a portal apears!", ch, NULL, NULL, TO_ROOM );
    			else
			act( AT_COLORIZE, skill->hit_room, ch, NULL, victim, TO_ROOM );
    			if ( !skill->hit_vict || skill->hit_vict[0] == '\0' )
    			echo_to_room( AT_MAGIC , ship->entrance, "a glowing white line apears, and twists, until a portal apears!");
    			else
    ...
    */
	...
	act( AT_COLORIZE, skill->hit_room, ch, NULL, victim, TO_ROOM );
    if ( !skill->hit_vict || skill->hit_vict[0] == '\0' )
	act( AT_MAGIC, "a glowing white line apears, and twists, until a portal apears!", victim, NULL, NULL, TO_ROOM );
    else
	act( AT_COLORIZE, skill->hit_vict, victim, NULL, victim, TO_ROOM );
    portalObj = obj_to_room( portalObj, ch->in_room );
