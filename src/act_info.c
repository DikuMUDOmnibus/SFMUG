...
char *format_obj_to_char( OBJ_DATA *obj, CHAR_DATA *ch, bool fShort )
{
	...
	if ( obj->short_descr )
     ...
     /* check to see if it is a gun, then show ammo */	    
       if( obj->item_type == ITEM_MISSILE_WEAPON && obj->value[5] > 3)
       {
	    sprintf( buf2, " (Ammo %d)", obj->value[3] );
	    strcat( buf, buf2 );
       }
     /* check to see if it is a clip, then show ammo */	    
       if( obj->item_type == ITEM_PROJECTILE && obj->value[0] > 0)
       {
	    sprintf( buf2, " (Ammo %d/%d)", obj->value[5], obj->value[0] );
	    strcat( buf, buf2 );
       }
    }
    else
...
*/
	...
	if (   (ship = ship_from_cockpit(ch->in_room))  != NULL )
	{
		...
		/* ship lost in space */
           else if ( ship->shipstate == SHIP_LOST )
		{
	        set_char_color(  AT_PURPLE, ch );
    	        ch_printf( ch , "\n\rPatterns of swirling energy.\n\r" );
	        set_char_color(  AT_RED, ch );
    	        ch_printf( ch , "\n\r\n\rYou are lost in space.\n\r" );
		}
           /* in a hanger */
           else if ( ship->on_ship && ( ship->location == 0 ) )
	      {
	          to_room = get_ship_hanger(ship->on_ship);
	          ch_printf( ch, "\n\r" );
	          original = ch->in_room;
	          char_from_room( ch );
...
void do_commands( CHAR_DATA *ch, char *argument )
{
	...
	for ( hash = 0; hash < 126; hash++ )
		...
		&&   strncmp( "do_mp", skill_name( command->do_fun ), 5 ) )
		{
