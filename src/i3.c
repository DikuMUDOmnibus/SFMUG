   ...
   if( reconnect )
   {
	...
	else if( reconattempts <= 20 )
	{
	   i3wait = 5000; /* Wait for 5000 game loops */
         i3log( "%s", "Will attempt to reconnect in approximately 5 minutes due to extended failures." );
	}
	else
	{
   ...
   switch( type )
   {
	 ...
	 break;
      case 'N': /* Originator's name */
         strncpy( code, oname, 50 );
	   code[50] = '\0';
         break;
      case 'O': /* Target's name */
         strncpy( code, tname, 50 );
	   code[50] = '\0';
         break;
   }
...
int I3_process_tell( char *s ) 
{
   ...
   I3_get_field( ps, &next_ps );
   I3_remove_quotes( &ps );
   i3_printf( ch, "&B%s&W@&B%s tells you&w: ", ps, header.originator_mudname );
...
void I3_tell( CHAR_DATA *ch, char *argument )
{
   ...
   i3_printf( ch, "&BYou tell %s&W@&B%s&w: &W%s\n\r", to, pmud->name, argument );
...
int I3_process_locate_reply( char *s ) 
{
   ...
   I3_get_field( ps, &next_ps );
   I3_remove_quotes( &ps );
   sprintf( mud_name, "%s", ps );
   ps = next_ps;

   I3_get_field( ps, &next_ps );
   I3_remove_quotes( &ps );
   sprintf( user_name, "%s", ps );
   ps = next_ps;
   ...
   for( ; ; )
   {
	...
	if( !str_cmp( word, "I3CONFIG" ) )
	{
#if defined(I3CHRONICLES)
	    char mudlib_buf[MSL];
	    sprintf( mudlib_buf, "%s %s.%s", CODEBASE_VERSION_TITLE, CODEBASE_VERSION_MAJOR, CODEBASE_VERSION_MINOR );
#endif
	   I3_fread_config_file( fin );
	    ...
	    this_mud->mudlib = I3STRALLOC( mudlib_buf );
#endif
	   continue;
	}
...
#ifdef SMAUGSOCIAL
   if( ( social = find_social( strlower( sname ) ) ) == NULL )
   {
