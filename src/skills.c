...
void do_slookup( CHAR_DATA *ch, char *argument )
{
	...
	ch_printf( ch, "Flags: %s  Guild: %d  Value: %d  Info: %d  Code: %s\n\r",
...
void do_backstab( CHAR_DATA *ch, char *argument )
{
    ...
    {
       if ( ( obj->value[4] == WEP_SWORD && obj->value[3] != DAM_PIERCE )
		|| obj->value[4] != WEP_SWORD )
       {
          send_to_char( "You need to wield a piercing or stabbing weapon.\n\r", ch );
          return;
       }
...
OBJ_DATA *find_projectile( CHAR_DATA *ch, int type )
{
    ...
    /* check to see if the gun is already loaded */
	...
	if ( can_see_obj(ch, obj) )
	{
	    ...
	    {
	        /* check to see if it is a clip, and if there are any rounds left     *
	         * basically it will either have rounds, or the max rounds will be 0  *
	         * 'cause an arrow won't have ammo. duh!!  -Lews                      */
	         /*    current ammo    ||     max ammo        */
	        if ( obj->value[5] > 0 ||  obj->value[0] < 1 )
	        {
	             return obj;
	        }
	    }
	...
	OBJ_DATA *projectile, sh_int dist, sh_int dt, char *stxt, sh_int color )
{
	...
	if ( projectile )
	{
		 ...
		 break;
		 case 17:
               learn_from_failure( ch, gsn_slugs );
		 break;
		 case 18:
               learn_from_failure( ch, gsn_blasters );
		 break;
		 case 19:
               learn_from_failure( ch, gsn_slances );
		 break;
	     }
    ...
    */
	...
	case 17:	wtype = gsn_slugs;	break;
	case 18:	wtype = gsn_blasters;	break;
	case 19:	wtype = gsn_slances;	break;
    }
 ...
 *
 * Shoot a projectile from a missile weapon (bow, crossbow, etc)
 *
...
void do_shoot( CHAR_DATA *ch, char *argument )
{
    ...
    if ( argument[0] == '\0' || !str_cmp(argument, " ") )
    {
	send_to_char( "Shoot where at who?\n\r", ch );
	return;
    }
   ...
   /* check to see if there is anything to shoot */
   /* look for a clip .. or a */
   for ( arrow = bow->last_content; arrow; arrow = arrow->prev_content )
   {  
       if( arrow->in_obj == bow )
       {
          if(bow->value[3] <= 0)
   ...
   if ( (arrow=find_projectile(ch, bow->value[5])) == NULL )
   {
	char *msg = "You have nothing to fire...\n\r";
      ...
      switch( bow->value[5] )
      {
        ...
        case PROJ_BULLET: msg = "You have no bullets...\n\r";	        break;
        case PROJ_BLAST: msg =  "You have no charges left...\n\r";	break;
        case PROJ_SLANCE: msg = "You have no charges left...\n\r";	break;
      }
	send_to_char( msg, ch );
	return;
   }
     ...
     if(arrow->value[0] > 0)
     {
        act( AT_MAGIC, "Lock and Load...", ch, NULL, NULL, TO_CHAR );
	act( AT_MAGIC, "$n says Lock and load!...", ch, NULL, NULL, TO_ROOM );
	/* They have to go dig out a clip at this point! */
	WAIT_STATE( ch, 8 );
        do_reload(ch, NULL);
     }
...
void do_reload( CHAR_DATA *ch, char *argument )
{
   ...
   /* find something to reload */
   if ( arg[0] == '\0' )
   {	
        if ( (wield = get_eq_char( ch, WEAR_MISSILE_WIELD )) == NULL )
        {
	    send_to_char("Reload what?\n\r", ch );
	    return;
	}
   }
   else if ( ( wield = get_obj_carry( ch, arg ) ) == NULL )
   {
	send_to_char("You do not have that.\n\r", ch );
	return;
   }
   
   /* check to see if it is already loaded, and take out the old clip */
       for ( obj = wield->last_content; obj; obj = obj->prev_content )
       {
               if(obj->in_obj == wield)
               {
                  separate_obj( obj );
                
                  /* make sure the clip has the right amount of ammo */
                  obj->value[5] = wield->value[3];
                  wield->value[3] = 0;
                
                  act( AT_ACTION, "$n unloads a $p.",ch, wield, NULL, TO_ROOM);
        	  act( AT_ACTION, "You unload a $p.",ch, wield, NULL, TO_CHAR);
	          obj_from_obj( obj);
        	  obj_to_room( obj, ch->in_room );
                  act( AT_ACTION, "$n dropa a $p.",ch, obj, NULL, TO_ROOM);
        	  act( AT_ACTION, "You drop a $p.",ch, obj, NULL, TO_CHAR);
     ...
     /* find a fresh clip */
     if ( (clip=find_projectile(ch, wield->value[5])) == NULL )
     {
	char *msg = "You have nothing to fire...\n\r";

      switch( wield->value[5] )
      {
        case PROJ_BOLT:	 msg =  "You have no bolts...\n\r";	        break;
        case PROJ_ARROW: msg =  "You have no arrows...\n\r";	        break;
        case PROJ_DART:	 msg =  "You have no darts...\n\r";	        break;
        case PROJ_STONE: msg =  "You have no slingstones...\n\r";	break;
        case PROJ_BULLET: msg = "You have no clips...\n\r";	        break;
        case PROJ_BLAST: msg =  "You have no energy cells...\n\r";	break;
        case PROJ_SLANCE: msg = "You have no energy cells...\n\r";	break;
      }
	send_to_char( msg, ch );
	...
	return;
        ...
        /* make sure they can't use arrows or sling stones as ammo clips. -Lews */
        if( clip->value[4] == PROJ_ARROW || clip->value[4] == PROJ_STONE )
        {
	    send_to_char("Reload what!?\n\r", ch );
	    return;
	}
 	...
 	/* make sure the gun has the right amount of ammo */
 	separate_obj(clip);
 	wield->value[3] = clip->value[5];
 	clip->value[5] = 0;
 	
 	act( AT_MAGIC, "$n slaps a $P in a $p.",ch, wield, clip, TO_ROOM);
        act( AT_MAGIC, "You slap a $P in your $p.",ch, wield, clip, TO_CHAR);
	obj_from_char( clip );
	obj_to_obj( clip, wield );
	save_char = TRUE;
