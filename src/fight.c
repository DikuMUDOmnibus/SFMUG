...
int weapon_prof_bonus_check( CHAR_DATA *ch, OBJ_DATA *wield, int *gsn_ptr )
{
    ...
    bonus = 0;	*gsn_ptr = gsn_pugilism; /* Change back to -1 if this fails horribly */
    if ( !IS_NPC(ch) && wield )   
    {
	switch(wield->value[4])
	{
	     /* Restructured weapon system - Samson 11-20-99 */
           default:             *gsn_ptr = -1;                  break;
           case WEP_BAREHAND:   *gsn_ptr = gsn_pugilism;        break;
           case WEP_SWORD:      *gsn_ptr = gsn_swords;          break;
           case WEP_DAGGER:     *gsn_ptr = gsn_daggers; 	break;
           case WEP_WHIP:       *gsn_ptr = gsn_whips;   	break;
           case WEP_TALON:      *gsn_ptr = gsn_talonous_arms;   break;
           case WEP_MACE:       *gsn_ptr = gsn_maces_hammers;   break;
           case WEP_ARCHERY:    *gsn_ptr = gsn_archery;         break;
           case WEP_BLOWGUN:    *gsn_ptr = gsn_blowguns;        break;
           case WEP_SLING:      *gsn_ptr = gsn_slings;          break;
           case WEP_AXE:        *gsn_ptr = gsn_axes;            break;
           case WEP_SPEAR:      *gsn_ptr = gsn_spears;          break;
           case WEP_STAFF:      *gsn_ptr = gsn_staves;          break;
           case WEP_SLUG:       *gsn_ptr = gsn_slugs;           break;
           case WEP_BLASTER:    *gsn_ptr = gsn_blasters;        break;
           case WEP_SLANCE:     *gsn_ptr = gsn_slances;         break;
	}
	if ( *gsn_ptr != -1 )
	  bonus = (int) ((LEARNED(ch, *gsn_ptr) -50)/10);
...
ch_ret one_hit( CHAR_DATA *ch, CHAR_DATA *victim, int dt )
{
    ...
    else if ( (wield = get_eq_char( ch, WEAR_MISSILE_WIELD )) != NULL )
    {
...
ch_ret damage( CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt )
{
	...
	if ( dt == gsn_poison || IS_POISON(dt) )
	  ...
	  /* Removed excess duplication, added hack and lash RIS types - Samson 1-9-00 */
        ...
        if ( dt == (TYPE_HIT + DAM_SLASH) )
          dam = ris_damage(victim, dam, RIS_SLASH);
	  else
	  if ( dt == (TYPE_HIT + DAM_HACK) )
	    dam = ris_damage(victim, dam, RIS_HACK);
	  else
	  if ( dt == (TYPE_HIT + DAM_LASH) )
	    dam = ris_damage(victim, dam, RIS_LASH);
	  else
	  if ( dt == (TYPE_HIT + DAM_SLUG) )
	    dam = ris_damage(victim, dam, RIS_SLUG);
	  else
	  if ( dt == (TYPE_HIT + DAM_BLAST) )
	    dam = ris_damage(victim, dam, RIS_BLAST);
	  else
	  if ( dt == (TYPE_HIT + DAM_SLANCE) )
	    dam = ris_damage(victim, dam, RIS_SLANCE);
...
void check_killer( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	if ( !IS_NPC( ch ) )
	{
	  ...
	  ch->pcdata->mkills++;
       /* Cheack to see if we are on a ship -Lews 9/01 */
...
/* backup in case hp goes below 1 */
    ...
    if ( victim->in_room && IS_NPC(victim) && victim->in_room->area && victim->in_room->area->planet )
    {
       ...
       else if ( xIS_SET(victim->act, ACT_CITIZEN ) )
       {
...
void make_corpse( CHAR_DATA *ch, CHAR_DATA *killer )
{
	...
	if ( ch->gold > 0 )
	{
	    if ( ch->in_room )
	    {
               /* this area stuff does not work on ships -Lews
