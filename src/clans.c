...
{   
    ...
    ch_printf_color( ch, "&wNumber2  : &W%-19.19s\t&wRank: &W%s\n\r",
			clan->number2,
			clan->tworank );
    ch_printf( ch, "&wAt War With: &W%-19.19s\t\r", 
                        clan->atwar );
    ...
    ch_printf_color( ch, "&wMembers  : &W%-6d  &wMemLimit: &W%-6d   &wAlign  : &W%-6d",
    			...
    			clan->mem_limit,
    			clan->alignment );
    ch_printf( ch, "&wSpacecraft     :         &W%-6d\n\r",
    			clan->spacecraft);
    ch_printf( ch, "&wVehicles       :         &W%-6d\n\r",
    			clan->vehicles);
    ch_printf( ch, "&wPlanets Controlled:      &W%-6d\n\r",
    			pCount);
    ch_printf( ch, "&wAverage Popular Support: &W%-6d\n\r",
    			support);
    ch_printf( ch, "&wMonthly Revenue:         &W%-6d\n\r",
    			revenue);
    ch_printf( ch, "&wHourly Wages:            &W%-6d\n\r",
    			clan->salary);
    ch_printf( ch, "&wTotal Funds:             &W%-6d\n\r",
    			clan->funds );
    if ( clan->clan_type == CLAN_GUILD )
    ...
    if ( argument[0] == '\0' )
    {
		...
		(clan->pkills[2]+clan->pkills[3]+
		 clan->pkills[4]+clan->pkills[5]) );
            ch_printf( ch, "          &rplanets Controled :  &w %-3d   &rRevenue:  &w%d\n\r",
                  pCount, revenue );
            count++;
        }
        set_char_color( AT_BLOOD, ch );
...
*/
    ...
    ||   nifty_is_name( ch->name, ch->pcdata->clan->leader  ))
	;
    else
    {
...
void do_overthrow( CHAR_DATA *ch , char * argument )
{
    ...
    if ( ch->pcdata->clan->leader && ch->pcdata->clan->leader[0] != '\0' )
    {
