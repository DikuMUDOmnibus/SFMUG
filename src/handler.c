...
/*
 * Updated exp_level function, to solve the pointless use of big numbers. -Orion
 */
int exp_level( CHAR_DATA *ch, sh_int level )
{
    ...
    lvl = UMAX( 0, (level - 1) );
    exp_level = ( get_exp_base( ch ) * ( (3 * lvl * lvl) + (3 * lvl) + 1 ) );
...
void char_to_room( CHAR_DATA *ch, ROOM_INDEX_DATA *pRoomIndex )
{
    ...
    /* Will this mess up because if ships? vnum -1? -Lews
    if ( ch->home_vnum < 1 )
...
/* area stuff doesn't work in ships -lews */
    if ( !IS_NPC(ch) && !IS_SET( ch->in_room->room_flags, ROOM_SPACECRAFT ) )
...
char *extra_bit_name( EXT_BV *extra_flags )
{
    ...
    if ( xIS_SET(*extra_flags, ITEM_ANTI_PILOT)) strcat( buf, " anti-pilot" );
    return ( buf[0] != '\0' ) ? buf+1 : "none";
}
