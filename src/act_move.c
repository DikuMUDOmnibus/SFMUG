...
/*
 * Function to get an exit, leading the the specified ship room
 */
EXIT_DATA *get_ship_exit_to( ROOM_INDEX_DATA *room, sh_int dir )
{
    EXIT_DATA *xit;
...
ch_ret move_char( CHAR_DATA *ch, EXIT_DATA *pexit, int fall )
{
	 ...
	 * area, but allow them to move around if already inside one. - Blodkai
         * It was crashing the mud when moving though ship rooms -Lews 
	 *//*
...
void do_leave( CHAR_DATA *ch, char *argument )
{
	...
	//  Take care of this in do_leaveship -lews
